...
function bookSlot(vehicle) {
  ...
  if (!find) {
    let message =
      "😔 Sorry! We haven't any slot for your vehicle" + vehicleType;
    showAlertBox(message, "danger");
  }

  // if Parking Place found
  else {
    console.log(`${vehicle.owner}, Sir You got Slot for your ${vehicle.type}`);
...
vehicleList.onclick = (event) => {
  ...
  if (event.target.nodeName === "LI") {
    ...
    selected_vehicle.src = `./img/service/${type}.png`;
...
const showPromo = (promo, price, pay, pricing) => {
  ...
  if (promo.value.toUpperCase() == "PABLO30") {
    price.innerHTML = `₹ ${pricing[type] * 0.7}.00/-`;
    pay.innerHTML = `₹ ${pricing[type] * 0.7}.00/-`;
